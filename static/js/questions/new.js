...
document.addEventListener('alpine:init', () => {
  Alpine.data('solviQuestionNew', () => ({
    ...
    async submit() {
      this.loading = true;
      try {
        const tags = this.form.tagsText.split(',').map((t) => t.trim()).filter(Boolean);
        ...
        if (!res.ok) {
          const msg = await res.json().catch(() => ({ error: '作成に失敗しました' }));
          window.notice.show({ message: msg.error || '作成に失敗しました', type: 'error' });
          return;
        }
