...
func (h *Handler) UpdateQuestion(c *echo.Context) error {
	...
	var due *time.Time
	if req.AnswerDue != nil && *req.AnswerDue != "" {
		t, err := time.Parse(time.RFC3339, *req.AnswerDue)
		if err != nil {
	...
	uuid := c.Param("uuid")
	if err := h.deps.Question.Update(claims.UserID, claims.IsSupporter, uuid, req.Status, due, req.Tags, req.IsRequireHumanSupport, req.Complete); err != nil {
		return c.JSON(http.StatusForbidden, map[string]string{"error": err.Error()})
	}
	q, _ := h.deps.Question.Get(claims.UserID, claims.IsSupporter, uuid)
	h.deps.Hub.SendToQuestion("update-question", q, q.QuestionUserID)
